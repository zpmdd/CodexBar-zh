...
struct CodexBarLocalizationTests {
    ...
    func `translates dynamic usage and account lines`() {
        Self.withChinese {
            ...
            #expect(CodexBarL10n.tr("5月5日 11:47: 8% used") == "5月5日 11:47：已用 8%")
    ...
    @Test
    func `language preference falls back to English`() {
        AppLanguageRuntime.withPreference(.english) {
            #expect(CodexBarL10n.tr("Settings...") == "Settings...")
            #expect(CodexBarL10n.tr("Untranslated upstream text") == "Untranslated upstream text")
        }
    }

    private static func withChinese(_ assertions: () -> Void) {
